...
\begin{exer2}[Vowels in alphabetical order]~\\
...
\begin{exer2}[Floating point numbers]\label{exer:floatingpoint}
~\\Let 
...
\begin{stmt}[Optional - Additional properties of languages]\label{exer:opproprl}
...
\exernfaOne
\end{exer1}    % Second-to-last letter is 'a'
...
\end{thm3}\footnotetext{For the purposes of this course, it will be sufficient for you to show a construction that is reasonably convincing and intuitively correct. Time permitting, we go over a proof of its correctness together in class.}
...
\draw [black] (23.5,-10.6) circle (2.4);
...
\draw [black] (19.3,-23.2) circle (2.4);
...
\begin{thm3}[Pumping lemma for regular languages]
...
\end{thm1}\footnotetext{Intuitively, a string of parentheses is balanced if each left parenthesis has a matching right parenthesis and the matched pairs are properly nested.}
...
% comment: could define left-most derivations and then ask if it's possible to have more than one *left-most* derivation for the strings in exer:cfgenglish
...
\item $\setbuild{w \in \setstar{\setdef{\charA, \charB}}}{w~\textrm{has an equal number of \charA{s} and \charB{s}}}$
   % ... already done in the previous exercise, but here to see if anyone paying attention
...
A \defterm{Turing machine} (\defterm{TM}) is a 7-tuple $(Q, \Sigma, \Gamma, \delta, q_0, \qaccept,\qreject )$, where $Q, \Sigma,$ and $\Gamma$ are all finite sets, and
\begin{enumerate}
\item $Q$ is the set of states,
